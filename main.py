        ...
        for proc in psutil.process_iter():
            if proc.name() == "totem":
                for totem in proc.as_dict()["open_files"]:
                    path = totem.path
                    if (
                        path.startswith("/home/ivan/Видео/Лекции")
        ...
        if check_py():
            if check_device():
                ...
                while True:
                    ...
                    if not check_device() or not check_py():
...
def making_note(day, full_data, time_of_start, time_of_end, proc, second, lock):
    ...
    filename = "timer.xlsx"
    font = Font(
        name="Calibri", size=20, bold=True, italic=False, strike=False, color="FF000000"
    )
    alignment = Alignment(
        horizontal="general",
        vertical="center",
    ...
    try:
        ...
        ws.append(
            ["DAY", "FULL_DATA", "TIME_OF_START", "TIME_OF_END", "PROC", "SECOND"]
        )
        ws.append([day, full_data, time_of_start, time_of_end, proc, second])
    finally:
        columns = ["A", "B", "C", "D", "E", "F"]
...
def main():
    ...
    task_1 = threading.Thread(
        name="check_video", target=get_time_video, daemon=True, args=(lock,)
    )
